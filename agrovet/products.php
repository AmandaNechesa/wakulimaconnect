...
if (isset($_POST['name'])) {
    $results = $connection->search([strtoupper($_POST['name'])]);
}
//cart
...
<head>
    ...
    <!-- Title  -->
    <title>WAKULIMA CONNECT PRODUCTS</title>
    <!-- Favicon  -->
    ...
    <div class="classy-nav-container breakpoint-off d-flex align-items-center justify-content-between">
        ...
        <div class="header-meta d-flex clearfix justify-content-end" style="background: #28a745;">
            ...
            <div class="favourite-area" style="background: #28a745; ">
                <a href="#"><img src="img/core-img/settings.svg" alt=""></a>
            </div>
            <!-- User Login Info -->
            <?php
...
<section class="new_arrivals_area section-padding-80 clearfix">
    ...
    <div class="container">
        <div class="row">
            <?php
            if (isset($_GET['id'])) {
                $sql = "SELECT * FROM wakulimaconnectagrovetitems WHERE id=" . $_GET['id'];
                $results = $connection->conn->query($sql);
            }
            if (isset($_GET['category'])) {
                $sql = "SELECT * FROM wakulimaconnectagrovetitems WHERE categoryid=" . $_GET['category'];
                $results = $connection->conn->query($sql);
            }
            if (!isset($results)) {
                $results = $connection->fetchProducts();
            }
            if ($results->rowCount() < 1){
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                            <div class="section-heading text-center btn-outline-danger">
                                <h2>the product is not available</h2>
                            </div>
                        </div>
                    </div>
                </div>
